...
class Decision:
    ...
    def calcul_single_device_offload_single_device_single_task_arrive_rate(self, source_device, target_device, task):
        ...
        if self.comput_position[source_device, task] == target_device:
            return self.possible[source_device, task] * self.task_comput[0, task]
        else:
            return 0
    ...
    def calcul_single_device_single_task_process_time(self, device, task):
        """任务在device处处理的时间"""
        return self.task_comput[0, task] / self.device_comput[0, device]
    ...
    def calcul_task_transmit_time(self, source_device, target_device, task):
        ...
        if source_device == target_device:
            ...
            return self.task_cache[0, task] / self.bandwidth[source_device, target_device]
    ...
    def calcul_single_device_single_task_time(self, end_device, task):
        """用户在提出单个任务的期望延迟"""
        return self.calcul_transmit_comput_transmit_time(self.cache_position[end_device, task],
                                                         self.comput_position[end_device, task],
                                                         end_device, task)
