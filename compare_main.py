            ...
            for i in range(len(temp)):
                ...
                for j in range(N_test):
                    ...
                    if change_var == 'cloud_cache':
                        device = Device(model.N_cloud, model.N_FAP, model.N_user, model.N_task, cloud_cache * temp[i],
                        ...
                        device = Device(model.N_cloud, model.N_FAP, model.N_user, model.N_task, cloud_cache,
                                        fap_cache * temp[i], user_cache, cloud_comput, fap_comput, user_comput)
                    elif change_var == 'user_cache':
                        device = Device(model.N_cloud, model.N_FAP, model.N_user, model.N_task, cloud_cache, fap_cache,
                                        user_cache * temp[i], cloud_comput, fap_comput, user_comput)
                    elif change_var == 'cloud_comput':
                        device = Device(model.N_cloud, model.N_FAP, model.N_user, model.N_task, cloud_cache, fap_cache,
                                        user_cache, cloud_comput * temp[i], fap_comput, user_comput)
                    elif change_var == 'fap_comput':
                        device = Device(model.N_cloud, model.N_FAP, model.N_user, model.N_task, cloud_cache, fap_cache,
                                        user_cache, cloud_comput, fap_comput * temp[i], user_comput)
                    elif change_var == 'user_comput':
                        device = Device(model.N_cloud, model.N_FAP, model.N_user, model.N_task, cloud_cache, fap_cache,
                                        user_cache, cloud_comput, fap_comput, user_comput * temp[i])
                    else:
                        pass
